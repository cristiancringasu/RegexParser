...
def kleen_nfa(nfa: NFA) -> NFA:
    translate_nfa(nfa, 1)
    nfa.numberOfStates += 2
    nfa.delta[(0,EPSILON)] = set([nfa.initialState, nfa.numberOfStates - 1])
    for state in nfa.finalStates:
        if (state, EPSILON) in nfa.delta.keys():
            nfa.delta[(state, EPSILON)].add(nfa.initialState)
            nfa.delta[(state, EPSILON)].add(nfa.numberOfStates - 1)
        else:
            nfa.delta[(state, EPSILON)] = set([nfa.initialState, nfa.numberOfStates - 1])


    nfa.initialState = 0
    nfa.finalStates = set([nfa.numberOfStates - 1])
    return nfa
